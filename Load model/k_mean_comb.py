...
def single_layer_kmean(root_codebook_size, weight_dict):
    ...
    for i, key in enumerate(layer_weight_dict):
        ...
        kmean = KMeans(n_clusters = codebook_size, init = "k-means++", n_init = 5, max_iter = 300, precompute_distances = True, verbose = 0, n_jobs = -1,algorithm = "full")
    ...
    for key in new_weight_dict:
        ...
        length1 = layer_shape_dict[key][0]
        data1 = data[:length1]
        # data2 = data[length1:]
...
def all_layer_kmean(root_codebook_size, weight_dict):
    length_list = []
    layer_weight_dict = {}
    layer_shape_dict = {}
    ...
    kmean = KMeans(n_clusters = codebook_size, init = "k-means++", n_init = 5, max_iter = 300, precompute_distances = True, verbose = 0, n_jobs = -1,algorithm = "full")
    kmean.fit(np.array(weight_all).reshape(-1, 1))
    ...
    for i, key in enumerate(weight_dict):
        ...
        data = new_weight[counter:counter+length]
        counter += length

        data_shape = list(ori_model_weight["model_state_dict"][key].shape)
        data = np.array(data).reshape(data1_shape)
...
def analy_comb(root_codebook_size, layer_list):
    ...
    modified_model.load_state_dict(ori_model_weight["model_state_dict"])
    modified_model.eval()
    ...
    if len(layer_list) != 1:
        all_weight_dict = all_layer_kmean(root_codebook_size, weight_dict)
    if single_weight_dict is not None:
        ...
        new_state_dict_single = None
    if all_weight_dict is not None:
        for key in all_weight_dict:
            new_state_dict_all[key] = all_weight_dict[key]

    return new_state_dict_single, new_state_dict_all


quantize_size = 8
root_codebook_size = 2**quantize_size
save_path = "state_dicts"
...
model_weights = {}
weight_bias_dict = {}
weight_bias_dict["l0"] = ["net.0.weight"]
weight_bias_dict["l1"] = ["net.4.weight"]
weight_bias_dict["l2"] = ["net.6.weight"]
weight_bias_dict["l3"] = ["net.10.weight"]
weight_bias_dict["l4"] = ["net.12.weight"]
weight_bias_dict["l5"] = ["net.16.weight"]
weight_bias_dict["l6"] = ["net.18.weight"]
weight_bias_dict["l7"] = ["net.22.weight"]
weight_bias_dict["l8"] = ["net.24.weight"]
weight_bias_dict["l9"] = ["net.28.weight"]
...
val_args = dict(shuffle=False, batch_size=2048, num_workers=8, drop_last=False) if cuda\
                    else dict(shuffle=True, batch_size=2048, drop_last=False, num_workers = 16)
...
run_list = [["l0"], ["l1"], ["l2"], ["l3"], ["l4"], ["l5"], ["l6"], ["l7"], ["l8"], ["l9"],]


for layer in run_list:
    print("Start training layer {}".format(str(layer)))
    state_single, state_all = analy_comb(root_codebook_size, layer)
    torch.save({"model_state_dict_single": state_single, 
                "model_state_dict_all": state_all}, "state_dicts/size_{}_layer_{}".format(quantize_size, str(layer)))
